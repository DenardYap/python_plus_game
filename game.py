...
screen = pygame.display.set_mode((1200, 700)) #screen instance
                                #pass in a tuple here (can also be a list actually, but usually people use tuple)
# loading images and rects
...
pikachu_new_rect = animation_list[0].get_rect(topleft = (100, 560))
count = 0
...
while True: 
    ...
    for event in events: # -> this will return a list []
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_a:
                ...
                if count >= 4:
                    count = 0
                if pikachu_new_rect.x >= 0:
                    pikachu_new_rect.x -= 3
            if event.key == pygame.K_d:
                count += 1
                if count >= 4:
                    count = 0
                if pikachu_new_rect.x <= 1100:
                    pikachu_new_rect.x += 10
            if event.key == pygame.K_SPACE:
                pikachu_new_rect.y -= 10 
    ...
    screen.blit(animation_list[int(count)], pikachu_new_rect) #method of screen instance 
    screen.blit(welcome_text, (300, 100))
    if pikachu_new_rect.colliderect(goomba_rect) == True:
        print("Game over!")
        gameovered += 1
        if gameovered == 1: 
            hit_sound.play()
